...
import NotionGalleryLoading from "@/components/notion/gallery/loading";
...
const Client = () => {
  const { notionBlog, state, goToPage, goBack } = useBlogListContext();

  if (notionBlog?.isFetching) {
    return (
      <NotionGalleryLoading length={notionBlog?.data?.results.length || 1} />
    );
  }
