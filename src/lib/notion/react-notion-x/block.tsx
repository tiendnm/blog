...
export const Block: React.FC<BlockProps> = (props) => {
  ...
  switch (block.type) {
    ...
    case "sub_sub_header": {
      ...
      const _link = slugify(block.properties.title.join(" ").trim());
      const innerHeader = (
        <span className="flex items-center">
          {/* <div id={id} className="notion-header-anchor" /> */}
          <div id={_link} className="notion-header-anchor" />
          {!block.format?.toggleable && (
            <a className="notion-hash-link" href={`#${_link}`} title={title}>
              <LinkIcon />
            </a>
          )}
              ...
              <div className="notion-bookmark-link">
                {block.format?.bookmark_icon && (
                  <div className="notion-bookmark-link-icon">
                    <LazyImage
                      src={
                        mapImageUrl(block.format?.bookmark_icon, block) || ""
                      }
                      alt={title}
                    />
                  </div>
    ...
    case "table_of_contents": {
      ...
      return (
        ...
        >
          {toc.map((tocItem) => {
            const _link = slugify(tocItem.text);
            return (
              <a
                key={tocItem.id}
                href={`#${_link}`}
                // href={`#${uuidToId(tocItem.id)}`}
                className="notion-table-of-contents-item pl-5"
              >
                <span
                  className="notion-table-of-contents-item-body !text-secondary-foreground"
                  style={{
